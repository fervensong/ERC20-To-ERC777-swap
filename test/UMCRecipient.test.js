...
const UMCRecipient = contract.fromArtifact('UMCRecipient');

describe('UMCRecipient', function () {
  ...
  beforeEach(async function () {
    ...
    this.recipient = await UMCRecipient.new(this.token.address, { from: creator });
  });
  ...
  it('sends to a contract from an externally-owned account', async function () {
    ...
    await expectEvent.inTransaction(receipt.tx, UMCRecipient, 'DoneStuff', { from: holder, to: this.recipient.address, amount: amount, userData: data, operatorData: null });
